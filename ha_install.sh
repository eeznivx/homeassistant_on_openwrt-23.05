...
# -----------------------------------------------------------------------
# Package manager abstraction: supports both apk (OpenWrt 25+) and opkg
# -----------------------------------------------------------------------
PKG_MANAGER=""
detect_pkg_manager() {
...
# Install a package; fail if not found
pkg_install() {
  if [ "$PKG_MANAGER" = "apk" ]; then
    apk add "$@"
  else
...
if [ -f /usr/lib/python${PYTHON_VERSION}/site-packages/botocore-*-info/METADATA ]; then
  ...
  # boto3/botocore egg-info may not exist if package is not installed (optional in 25.12)
  [ -f /usr/lib/python${PYTHON_VERSION}/site-packages/botocore-*.egg-info/requires.txt ] && \
    sed -i 's/urllib3<1.25,>=1.20/urllib3>=1.20/' /usr/lib/python${PYTHON_VERSION}/site-packages/botocore-*.egg-info/requires.txt || true
  [ -f /usr/lib/python${PYTHON_VERSION}/site-packages/boto3-*.egg-info/requires.txt ] && \
    sed -i 's/botocore<1.13.0,>=1.12.135/botocore<1.13.0,>=1.12.0/' /usr/lib/python${PYTHON_VERSION}/site-packages/boto3-*.egg-info/requires.txt || true
fi
[ -d /usr/lib/python${PYTHON_VERSION}/site-packages/pycountry ] && \
  rm -rf /usr/lib/python${PYTHON_VERSION}/site-packages/pycountry/{locales,tests} || true
